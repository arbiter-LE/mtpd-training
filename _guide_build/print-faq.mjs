/* Render client-faq.html -> PDF via headless Chrome (DevTools Page.printToPDF).
   Build-time tool — client-facing draft, review before sending. */
...
const HTML = pathToFileURL(fileURLToPath(new URL('./client-faq.html', import.meta.url))).href;
const OUT = fileURLToPath(new URL('./Arbiter-LE-FAQ.pdf', import.meta.url));
const PORT = 9349;
...
const chrome = spawn(CHROME, ['--headless=new', `--remote-debugging-port=${PORT}`, '--disable-gpu', '--no-first-run', '--no-default-browser-check', `--user-data-dir=/tmp/faq-pdf-${Date.now()}`, HTML], { stdio:'ignore' });
...
const footer = `<div style="font-size:8px;width:100%;padding:0 0.5in;color:#5a7a92;display:flex;justify-content:space-between;font-family:Arial">
  <span>Arbiter LE &nbsp;·&nbsp; Frequently Asked Questions</span>
  <span>arbiterle.com</span>
</div>`;
