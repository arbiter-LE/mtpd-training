/* Headless-Chrome screenshot capture for the MTPD user guide.
   ...
   MTPD uses the default :root (navy/steel/gold) theme. */
...
const APP_URL = 'http://localhost:3000/?dept=mtpd';
const OUT = fileURLToPath(new URL('./screens-mtpd/', import.meta.url));
const PORT = 9335;
...
const BASE_SETUP = `
  ...
  currentUser = { id:'MTPD-DEMO', name:'Ofc. M. Reyes', rank:'Patrol Officer', role:'officer', track:'patrol', badge:'MTPD-204', email:'m.reyes@example.gov' };
  USERS['MTPD-DEMO'] = Object.assign({}, currentUser);
  completionData['MTPD-DEMO'] = {
    'search-seizure': { attempts:1, passed:true, bestScore:92, score:92, scores:[92], date:'2026-05-19', correct:11, total:12, time:'14:22' },
    'use-of-force':   { attempts:2, passed:true, bestScore:88, score:88, scores:[64,88], date:'2026-05-31', correct:14, total:16, time:'18:05' },
    'report-writing': { attempts:1, passed:true, bestScore:100, score:100, scores:[100], date:'2026-06-07', correct:10, total:10, time:'11:40' }
  };
...
const ADMIN_SETUP = `
  currentUser = { id:'ADMIN-DEMO', name:'Administrator', rank:'Command Staff', role:'admin', track:'supervisor', badge:'MTPD-001', email:'admin@example.gov' };
  previewTrackOverride = null;
  delete USERS['MTPD-DEMO']; delete completionData['MTPD-DEMO'];
  var roster = [
    ['MTPD-204','Ofc. M. Reyes','Patrol Officer',  {1:[92,1],2:[88,2],3:[100,1],4:[84,1]}],
    ['MTPD-211','Ofc. T. Alvarez','Patrol Officer', {1:[78,1],2:[91,1],3:[88,1]}],
    ['MTPD-188','Sgt. K. Doyle','Sergeant',         {1:[95,1],2:[97,1],3:[100,1],4:[92,1],5:[89,1],6:[94,1]}],
    ['MTPD-220','Ofc. P. Nowak','Patrol Officer',   {1:[71,1],2:[58,3]}],
    ['MTPD-233','Ofc. D. Burke','Patrol Officer',   {1:[100,1]}]
  ];
...
const SCREENS = [
  ...
  { name: '02-forgot-password', setup: `
      ...
      document.getElementById('login-user').value='m.reyes@example.gov';
      var err=document.getElementById('login-error'); err.textContent='Reset link sent — check your email.'; err.style.color='var(--accent)';
  ` },
  ...
  ` },
  { name: '06-module-reading', setup: `${BASE_SETUP} startModule('search-seizure'); window.scrollTo(0,0);` },
  { name: '07-scenario-scene', setup: `${BASE_SETUP} startScenario('search-seizure'); window.scrollTo(0,0);` },
  { name: '08-scenario-decision', setup: `
      ${BASE_SETUP} startScenario('search-seizure');
  ...
  ` },
  { name: '10-quiz-question', setup: `${BASE_SETUP} startQuiz('search-seizure'); window.scrollTo(0,0);` },
  { name: '11-quiz-feedback', setup: `
      ${BASE_SETUP} startQuiz('search-seizure');
      var q=activeQuestions(currentModule)[0]; selectAnswer(q.correct); window.scrollTo(0,0);
  ` },
  { name: '12-results-pass', setup: `
      ${BASE_SETUP} currentModule = MODULES.find(function(m){return m.id==='search-seizure';});
      quizCorrect=11; quizTotal=12; quizSeconds=862; currentQuizIdx=12;
      delete completionData['MTPD-DEMO']['search-seizure'];
...
// ── main ─────────────────────────────────────────────────────────
// Launch a dedicated headless Chrome in its own detached process group.
// (Re-invoking the Chrome.app bundle from this script directly can hand off
//  to a running instance on macOS and silently drop the debug flags, so we
//  isolate it with a fresh user-data-dir and our own port.)
...
const chrome = spawn(CHROME, [
  ...
  `--user-data-dir=/tmp/mtpd-cdp-${Date.now()}`,
  `--window-size=${W},${H}`,
  APP_URL
